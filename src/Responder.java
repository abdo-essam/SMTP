...
public class Responder extends Thread
{
    ...
    Socket socket = null;
    String serverIP, serverHostName, clientAddress, clientHostName,received, response;
    int clientPort;
    ...
    public Responder(Socket socket, DataInputStream dis, DataOutputStream dos, String serverIP, String serverHostName) {
        super();
        this.socket = socket;
        this.dis = dis; 
        this.dos = dos; 
    ...
    private String processResponse(String theInput, String clientName)
    {
        ...
        /*
         ...
         the client authenticates itself to the server, giving its UserName and Password.
         It’s another layer of security to guarantee a proper transmission.  
         */
        else if (state == 2) {
            if (theInput.length() >= 4 
        ...
        // Need to handle if it's a new user as well here
            ...
            if (udbFolder.exists() && udbFolder.isDirectory())
            {
                ...
                File passFile = new File(System.getProperty("user.dir")+ File.separator+"db" +File.separator+".user_pass.txt");
                if (passFile.exists()) {
                    try {
        ...
        // Expecting Password in base64
            ...
            // check a00gainst password database
            File passFile = new File(System.getProperty("user.dir")+ File.separator+"db" +File.separator+".user_pass.txt");
            if (passFile.exists()) {
                try {
        ...
        // Expecting MAIL FROM
            ...
            if (theInput.length() >= 9 && theInput.substring(0, 9).equals("MAIL FROM")) {
                ...
                && (theInput.substring(10).equals(username + "@fci-cu.edu.eg")))
                {
                    theOutput = "250 <" + username + "fci-cu.edu.eg>... Sender ok";
                    state = 6;
                }
                else {
                    theOutput = "501 Invalid Entry, expecting email of form username@fci-cu.edu.eg";
                }
            }
            else {
                if (theInput.length() >= 4 && (theInput.substring(0, 4).equals("AUTH") || theInput.substring(0, 4).equals("DATA")
        ...
        // Expecting RCPT TO
            ...
            if (theInput.length() >= 7 && theInput.substring(0, 7).equals("RCPT TO")) {
                ...
                Pattern pattern = Pattern.compile("^(.+)@fci-cu.edu.eg$");
                Matcher matcher = pattern.matcher(theInput.substring(8));
                if (!matcher.matches())
                {
                    theOutput = "501 Invalid Entry, expecting email of form recipient@fci-cu.edu.eg";
                }
                else
                {
                    ...
                    theOutput = "250 <" + receivUser + "@fci-cu.edu.eg>... Recipient ok";
                    state = 7;
            ...
            if (theInput.length() >= 4 && theInput.substring(0, 4).equals("DATA")) {
                ...
                else
                {
                    ...
                    String email = ("Date: " + ts + " CST" + System.lineSeparator()
                            + "From: <" + username + "@fci-cu.edu.eg>" + System.lineSeparator()
                            + "To: <" + receivUser + "@fci-cu.edu.eg>" + System.lineSeparator()
    ...
    private void writeLog(String fromIP, String toIP, String msg)
    {
        ...
        try { 
            BufferedWriter out = new BufferedWriter(
                    new FileWriter(".server_log.txt", true));
